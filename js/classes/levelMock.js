class LevelMock {
  ...
  drawMap() {
    ...
    _.each(this.data.path, function (node) {
      ...
      that.path.lineTo(
        resolvePosition(node.x - 2) - 28,
        resolvePosition(node.y - 2) - 4
      );
    });

    scene.grid[this.data.start.x][this.data.start.y].setPath(0x157334);
    scene.grid[this.data.end.x][this.data.end.y].setPath(0x73331e);
  }
            ...
            switch (this.batch.type) {
              case "BlueTriangle":
                new Triangle(
                  ...
                  resolvePosition(scene.level.data.start.x - 2) - 12,
